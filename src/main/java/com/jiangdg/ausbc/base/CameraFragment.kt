...
abstract class CameraFragment : BaseFragment(), ICameraStateCallBack {
    ...
    private fun handleTextureView(textureView: TextureView) {
        // 1. Set the correct listener interface
        textureView.surfaceTextureListener = object : TextureView.SurfaceTextureListener {

            // 2. Implement the required abstract method for TextureView
            override fun onSurfaceTextureAvailable(
                surface: android.graphics.SurfaceTexture,
                width: Int,
                height: Int
            ) {
                // Analogous to surfaceCreated()
                registerMultiCamera()
                surfaceSizeChanged(width, height)
            }

            // 3. Implement the method analogous to surfaceChanged()
            ...
            // 4. Implement the method analogous to surfaceDestroyed()
            override fun onSurfaceTextureDestroyed(surface: android.graphics.SurfaceTexture): Boolean {
                unRegisterMultiCamera()
                // Return true to indicate ownership of the SurfaceTexture has been released
                return true
            }

            // 5. Implement the remaining required method
            override fun onSurfaceTextureUpdated(surface: android.graphics.SurfaceTexture) {
                // Called every time the SurfaceTexture is updated. Usually ignored for camera.
            }
        }
    }
    ...
    private fun handleSurfaceView(surfaceView: SurfaceView) {
        surfaceView.holder.addCallback(object : SurfaceHolder.Callback {
            override fun surfaceCreated(holder: SurfaceHolder) {
                // holder is now guaranteed to be non-null
                registerMultiCamera()
            }

            override fun surfaceChanged(
                holder: SurfaceHolder, // Changed from SurfaceHolder?
                format: Int,
                width: Int,
                height: Int
            ) {
            ...
            override fun surfaceDestroyed(holder: SurfaceHolder) { // Changed from SurfaceHolder?
                unRegisterMultiCamera()
            }
        })
    }
